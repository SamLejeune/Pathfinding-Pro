...
const JPSExpandHorizontally = function (originCell, dir) {
  ...
  for (let j = currentCell.y; j < max && j >= min; dir === 'for' ? j++ : j--) {
    ...
    if (currentCell.x < grid.columns - 1) {
...
export const DFSMaze = function () {
  ...
  while (true) {
    let calcX = Math.floor(Math.random() * grid.columns);
    let calcY = Math.floor(Math.random() * grid.rows);
...
const huntPhase = function () {
  ...
  for (let i = 1; i <= grid.columns - 1; i += 2) {
    for (let j = 1; j <= grid.rows - 1; j += 2) {
...
export const simpleRandomMaze = async function () {
  ...
  while (i < total) {
    ...
    randomX = Math.floor(Math.random() * grid.columns);
    randomY = Math.floor(Math.random() * grid.rows);
